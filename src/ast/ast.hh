...
public:
    ...
    RE(shared_ptr<ASTNode> child, bool capturing = false, string group_name = "RegEx");
...
public:
    LeafNode();
    virtual bool is_match(const string& ch = "", int str_i = -1, int str_len = -1) const;
};
...
public:
    ...
    bool is_match(const string& ch = "", int str_i = 0, int str_len = 0) const override;
    string match;
...
public:
    ...
    GroupNode(deque<shared_ptr<ASTNode>> children, bool capturing = false, string group_name = "", int group_id = -1);
    bool is_capturing() const;
};
