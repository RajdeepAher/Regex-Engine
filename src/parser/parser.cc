...
Token* Parser::next_tkn(bool without_consuming) {
...
function<Token*(bool)> Parser::next_tkn_initializer(const string& re) {
    vector<Token*> tokens = lxr.scan(re);
    int i = -1;

    return [tokens, i](bool without_consuming = false) mutable -> Token* {
        if (without_consuming) {
            return (i + 1 < tokens.size()) ? tokens[i + 1] : nullptr;
        }
        i++;
...
shared_ptr<ASTNode> Parser::parse_re_seq(bool capturing, const string& group_name, int group_id) {
    ...
    // Handle start anchor (^)
    if (dc<Start*>(curr_tkn) || dc<Circumflex*>(curr_tkn)) {
...
shared_ptr<GroupNode> Parser::parse_group(bool capturing, const string& group_name, int group_id) {
    ...
    while (curr_tkn && !dynamic_cast<OrToken*>(curr_tkn) &&
           !dynamic_cast<RightParenthesis*>(curr_tkn) &&
           !dynamic_cast<EndToken*>(curr_tkn)) {
        ...
        if (dynamic_cast<EndToken*>(curr_tkn)) {
        ...
        if (dynamic_cast<Quantifier*>(curr_tkn)) {
            ...
            if (auto element = dynamic_pointer_cast<Element>(new_el)) {
                if (dynamic_cast<ZeroOrOne*>(curr_tkn)) {
                    //cout << "ZeroOrOne quantifier." << endl;
                    element->min = 0;
                    element->max = 1;
                } else if (dynamic_cast<ZeroOrMore*>(curr_tkn)) {
                    //cout << "ZeroOrMore quantifier." << endl;
                    element->min = 0;
                    element->max = numeric_limits<double>::infinity();
                } else if (dynamic_cast<OneOrMore*>(curr_tkn)) {
        ...
        } else if (dynamic_cast<LeftCurlyBrace*>(curr_tkn)) {
            //cout << "Current token: " << (curr_tkn ? curr_tkn->char_ : "nullptr") << endl;
...
void Parser::parse_curly(shared_ptr<ASTNode> new_el) {
    ...
    //cout << "Current token: " << (curr_tkn ? curr_tkn->char_ : "nullptr") << endl;
    //cases: 2,} 2} 2,3} }
    if (dynamic_cast<RightCurlyBrace*>(curr_tkn)) {
    ...
    try {
        ...
        //cases: 2,} 2} 2,3}
        while (dc<ElementToken*>(curr_tkn)) {
            auto shit = dc<ElementToken*>(curr_tkn);
            val_1 += shit->char_;
            next_tkn();
            //cout << "Current token: " << (curr_tkn ? curr_tkn->char_ : "nullptr") << endl;
        }
        ...
        //cout << "Current token: " << (curr_tkn ? curr_tkn->char_ : "nullptr") << endl;
        while (dynamic_cast<ElementToken*>(curr_tkn)) {
            auto shit = dynamic_cast<ElementToken*>(curr_tkn);
            val_2 += shit->char_;
            next_tkn();
            //cout << "Current token: " << (curr_tkn ? curr_tkn->char_ : "nullptr") << endl;
        }
...
shared_ptr<ASTNode> Parser::parse_range_el() {
    if (dynamic_cast<LeftBracket*>(curr_tkn)) {
        next_tkn();

        auto element = parse_inner_el();
        if (dynamic_cast<RightBracket*>(curr_tkn)) {
            return element;
        } else {
...
shared_ptr<RangeElement> Parser::parse_inner_el() {
    ...
    while (curr_tkn) {
        if (dc<RightBracket*>(curr_tkn)) break;

        if (dc<SpaceToken*>(curr_tkn)) {
            match_str += dc<SpaceToken*>(curr_tkn)->char_;
            //cout << "This is the space token char in parse_inner_el() " << dc<SpaceToken*>(curr_tkn)->char_ << endl;
            next_tkn();
            continue;
        }

        if (!dynamic_cast<ElementToken*>(curr_tkn)) {
            curr_tkn = new ElementToken(dynamic_cast<Token*>(curr_tkn)->char_);
        }

        Token* peek_tkn = next_tkn(true);

        if (peek_tkn && dc<Dash*>(peek_tkn)) {
            prev_char = dc<ElementToken*>(curr_tkn)->char_[0];
            ...
            Token* next_peek_tkn = next_tkn(true);

            if (dc<RightBracket*>(next_peek_tkn) || dc<SpaceToken*>(next_peek_tkn)) {
                match_str += prev_char;
                match_str += dc<Dash*>(peek_tkn)->char_;
            } else {
                next_tkn();
                if (!curr_tkn) throw runtime_error("Missing closing ']'");

                char end_char = dc<ElementToken*>(curr_tkn)->char_[0];
            ...
            match_str += dc<ElementToken*>(curr_tkn)->char_;
            next_tkn();
        }
    }
...
shared_ptr<ASTNode> Parser::parse_el() {
    ...
    if (dynamic_cast<ElementToken*>(curr_tkn)) {
        ...
        return make_shared<Element>(dynamic_cast<ElementToken*>(curr_tkn)->char_);
    } else if (dynamic_cast<Wildcard*>(curr_tkn)) {
    ...
    } else if (dynamic_cast<SpaceToken*>(curr_tkn)) {
    ...
    } else if (dynamic_cast<LeftParenthesis*>(curr_tkn)) {
        ...
        if (dynamic_cast<QuestionMark*>(curr_tkn)) {
            //cout << "parse_el: Found QuestionMark, checking for non-capturing or named group" << std::endl;
            next_tkn(); // Move past the '?'

            if (dynamic_cast<ElementToken*>(curr_tkn) && dynamic_cast<ElementToken*>(curr_tkn)->char_ == ":") {
                // Debug: Non-capturing group
                //cout << "parse_el: Found ':', marking group as non-capturing" << std::endl;
                capturing = false;
                next_tkn(); // Move past the ':'
            } else if (dynamic_cast<ElementToken*>(curr_tkn) && dynamic_cast<ElementToken*>(curr_tkn)->char_ == "<") {
                // Debug: Named group
        ...
        if (dynamic_cast<RightParenthesis*>(curr_tkn)) {
...
string Parser::parse_group_name() {
    ...
    while (curr_tkn && dynamic_cast<ElementToken*>(curr_tkn) && dynamic_cast<ElementToken*>(curr_tkn)->char_ != ">") {
        group_name += dynamic_cast<ElementToken*>(curr_tkn)->char_;
        next_tkn();
    }
    ...
    //consume the > token
    if (curr_tkn && dynamic_cast<ElementToken*>(curr_tkn) && dynamic_cast<ElementToken*>(curr_tkn)->char_ == ">") {
        next_tkn();
    } else {
